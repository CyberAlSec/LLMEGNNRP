...
from augmodel.LMs.model import BertClassifier, BertClaInfModel
from data_utils.dataset import Dataset
from data_utils.load import load_data
from utils import init_path, time_logger, get_model_name, get_project_root
...
class LMTrainer():
    def __init__(self, cfg):
        ...
        self.model_name,self.dimension = get_model_name(cfg.feature_type)
        self.feat_shrink = ""

        self.weight_decay = cfg.lm_weight_decay
        self.dropout = cfg.lm_dropout
        self.att_dropout = cfg.lm_att_dropout
        self.cla_dropout = cfg.lm_cla_dropout
        self.batch_size = cfg.lm_batch_size
        self.epochs = cfg.lm_epochs
        self.warmup_epochs = cfg.lm_warmup_epochs
        self.eval_patience = cfg.lm_eval_patience
        self.grad_acc_steps = cfg.lm_grad_acc_steps
        self.lr = cfg.lm_lr
        root = get_project_root()
        self.use_gpt_str = "2" if cfg.lm_use_gpt else ""
        attack_name = "clean" if cfg.text_attack is None else cfg.text_attack
        model_name = self.model_name.split("/")[-1]
        if cfg.surrogate:
            self.output_dir = f'{root}/data/surr/{self.dataset_name}/{model_name}-seed{self.seed}'
            self.ckpt_dir = f'{root}/data/surr/{self.dataset_name}/{model_name}-seed{self.seed}'
        else:
            self.output_dir = f'{root}/data/prem/{cfg.feature_type}/{attack_name}/{self.dataset_name}{self.use_gpt_str}/{model_name}-seed{self.seed}'
            self.ckpt_dir = f'{root}/data/emb/{cfg.feature_type}/{attack_name}/{self.dataset_name}{self.use_gpt_str}/{model_name}-seed{self.seed}'

        # Preprocess data
        data, num_classes, text = load_data(
            dataset=self.dataset_name, use_text=True, use_gpt=cfg.lm_use_gpt, seed=self.seed)
        self.data = data
    ...
    def eval_and_save(self):
        emb = np.memmap(init_path(f"{self.ckpt_dir}.emb"),
                        dtype=np.float16,
                        mode='w+',
                        shape=(self.num_nodes, self.feat_shrink if self.feat_shrink else self.dimension))
