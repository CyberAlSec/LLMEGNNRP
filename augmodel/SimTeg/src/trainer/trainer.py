...
class Trainer(ABC):
    ...
    def inference(self, dataset, embs_path,attack):

        x_embs_name = f"{embs_path}/data/emb/SimTeg/{self.args.dataset}/SimTeg_{self.args.dataset}_{attack}_embs.pt"
    ...
    def inference_and_evaluate(self, dataset,attack):
        path = get_project_root()
        logits_embs, x_embs = self.inference(dataset, path,attack)
    ...
    def train(self, return_value="valid",attack=None):
        self.prepare()
        ...
        if attack is None:
            attack = "clean"
        _, _, results = self.inference_and_evaluate(self.all_set,attack)
        gc.collect()
        torch.cuda.empty_cache()
