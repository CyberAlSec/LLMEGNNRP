...
from attack.attackers.base import AttackABC
    ...
    def _compute_gradient(self, edges_all, weights, inputs, target_weight=None):
        ...
        if isinstance(inputs, (list, tuple)):
            for i, inp in enumerate(inputs):
                print(f"inputs[{i}].requires_grad: {inp.requires_grad}")
                print(f"inputs[{i}].grad_fn: {inp.grad_fn}")
        else:
            print(f"inputs.requires_grad: {inputs.requires_grad}")
            print(f"inputs.grad_fn: {inputs.grad_fn}")
...
class PoisonPGA(PGA):
    ...
    def __init__(self,
                 attack_config, pyg_data,
                 model, device,  **kwargs):
        super(PoisonPGA, self).__init__(attack_config, pyg_data, model, device,  **kwargs)

        self.surrogate = type(self.attacked_model)(
            self.attacked_model.config, pyg_data, device,
            with_relu=False)
