...
class GCN(nn.Module):
    ...
    def __init__(self, nfeat, nhid, nclass, use_pred,nlayers=2, dropout=0.5, lr=0.01, weight_decay=5e-4,
            with_relu=True, with_bias=True, device=None,attention=False):
        ...
        if self.use_pred:
            self.encoder = torch.nn.Embedding(nclass + 1, nhid).cuda()
