...
class SAGE(BaseModel):
    ...
    def __init__(self, nfeat, nhid, nclass,use_pred, num_layers=2,
                 dropout=0.5, lr=0.01, weight_decay=0, device='cpu', with_bn=True, **kwargs):
    ...
    def forward(self, x, adj_t,edge_weight=None):
        ...
        for i, conv in enumerate(self.convs[:-1]):
            x = conv(x, adj_t.to(torch.int64))
            x = self.bns[i](x)
            x = F.relu(x)
            x = F.dropout(x, p=self.dropout, training=self.training)
        x = self.convs[-1](x, adj_t.to(torch.int64))
