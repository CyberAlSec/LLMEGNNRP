...
def get_raw_text_pubmed(use_text=False,attack_type=None,seed=0):
    data = torch.load(f'{root}/data/pubmed.pt')
    text = data.text
    ...
    if not attack_type == None:
        advdata = torch.load(f"{root}/attack/data/orig/pubmed/pubmed_{attack_type}_orig.pt")
